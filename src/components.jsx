  ...
  return (
    <div className="app">
      <aside className="sidebar">
        <div className="brand">
          <div className="brandmark">PH</div>
          <div>
            <b>Pilotage Heures</b>
            <small>Contrôle industriel</small>
          </div>
        </div>
    ...
    <div className="tablewrap">
      <table>
          ...
          {lignes.map(r=>(
            <tr key={r.metier}>
              <td>
                <span
                  className="dot"
                  style={{background:COLORS[r.metier]}}
                ></span>
                <b>{r.metier}</b>
              </td>

              <td>{fmt(r.encouru)} h</td>
              <td>{fmt(r.budgetDate)} h</td>
              <td>{fmt(r.budgetAlloue)} h</td>
              <td>{fmt(r.reste)} h</td>

              <td>{pct(r.consoReelle)}</td>
              <td>{pct(r.consoDate)}</td>

              <td className={r.ecartH>0?"bad":r.ecartH<0?"good":""}>
                {sign(r.ecartH)} h
              </td>

              <td className={r.ecartPoints>0?"bad":r.ecartPoints<0?"good":""}>
                {sign(r.ecartPoints)}
              </td>
              ...
              <td>
                {onDetail && (
                  <button
                    ...
                    onClick={()=>nav(`/metier/${encodeURIComponent(r.metier)}`)}
                  >
                    Voir
                  </button>
                )}
        ...
        <tfoot>
          <tr>
            <td><b>TOTAL</b></td>
            <td>{fmt(total.encouru)} h</td>
            <td>{fmt(total.budgetDate)} h</td>
            <td>{fmt(total.budgetAlloue)} h</td>
            <td>{fmt(total.reste)} h</td>
            <td>{pct(total.consoReelle)}</td>
            <td>{pct(total.consoDate)}</td>
            <td>{sign(total.ecartH)} h</td>
            <td>{sign(total.ecartPoints)}</td>
            <td><Status status={total.statut}/></td>
            <td></td>
          </tr>
        </tfoot>
      </table>
